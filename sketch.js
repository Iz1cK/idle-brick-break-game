let balls = [];
let blocks = [];
let addRedBall;
let addGreenBall;
let moneyText;
function randomIntFromInterval(min, max) {
  return Math.floor(Math.random() * (max - min + 1) + min);
}

function setup() {
  createCanvas(400, 400);
  money = 0;
  for (let i = 0; i < 5; i++) {
    balls[i] = new Ball(randomIntFromInterval(20, width - 20), height - 20, 0);
  }
  addRedBall = createButton("Add Red Ball");
  addRedBall.mousePressed(() => {
    balls.push(new Ball(randomIntFromInterval(20, width - 20), 20, 1));
  });
  addGreenBall = createButton("Add Green Ball");
  addGreenBall.mousePressed(() => {
    balls.push(new Ball(randomIntFromInterval(20, width - 20), 20, 2));
  });
  moneyText = createSpan("Money:" + money);
  moneyText.id("money");
  let blockCords = [
    { x: 50, y: 90, id: 0 },
    { x: 50, y: 130, id: 0 },
    { x: 50, y: 170, id: 0 },
    { x: 50, y: 210, id: 0 },
    { x: 50, y: 250, id: 0 },
    { x: 75, y: 50, id: 0 },
    { x: 75, y: 290, id: 0 },
    { x: 100, y: 50, id: 0 },
    { x: 125, y: 50, id: 0 },
    { x: 150, y: 50, id: 0 },
    { x: 175, y: 50, id: 2 },
    { x: 100, y: 290, id: 0 },
    { x: 125, y: 290, id: 0 },
    { x: 150, y: 290, id: 0 },
    { x: 175, y: 290, id: 0 },
    { x: 200, y: 90, id: 0 },
    { x: 200, y: 250, id: 0 },
    { x: 200, y: 210, id: 0 },
    { x: 175, y: 210, id: 0 },
    { x: 150, y: 210, id: 1 },
  ];
  for (let i = 0; i < blockCords.length; i++) {
    blocks[i] = new Block(
      blockCords[i].x,
      blockCords[i].y,
      25,
      40,
      blockCords[i].id
    );
  }
}

function draw() {
  background(25);
  for (let i = 0; i < blocks.length; i++) {
    blocks[i].show();
  }
  for (let i = 0; i < balls.length; i++) {
    balls[i].show();
  }

  for (let i = 0; i < balls.length; i++) {
    for (let j = 0; j < blocks.length; j++) {
      let block = blocks[j];
      let ball = balls[i];
      ball.checkCollides(block);
    }
  }
}
